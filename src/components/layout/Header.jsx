...
const resumeButtonClassName =
  cn(type.button, 'group inline-flex cursor-pointer items-center gap-2 rounded-full bg-neutral-900 px-4 py-2 text-neutral-50 uppercase transition-colors hover:bg-neutral-800 dark:bg-neutral-100 dark:text-neutral-900 dark:hover:bg-neutral-200')

const headerControlClassName =
  'inline-flex items-center rounded-full border border-neutral-400 bg-[#f2efea]/70 p-1 text-neutral-800 shadow-sm shadow-neutral-900/5 backdrop-blur transition-colors dark:border-neutral-600 dark:bg-[#141414]/70 dark:text-neutral-200'
...
export default function Header() {
  ...
  return (
    <header className="border-x border-t border-neutral-300 px-5 pt-5 sm:px-8 sm:pt-8 lg:px-12 dark:border-neutral-700 ">
      <div className="flex items-center justify-between">
        ...
        <div className={cn(headerControlClassName, 'md:hidden')}>
          ...
          <button
            type="button"
            className="flex h-8 w-8 cursor-pointer items-center justify-center rounded-full text-neutral-900 transition-colors hover:bg-neutral-950/5 dark:text-neutral-100 dark:hover:bg-neutral-50/10"
      ...
      <nav
        className={cn(
          ...
          menuOpen ? 'mt-6 grid-rows-[1fr] opacity-100' : 'mt-0 grid-rows-[0fr] opacity-0',
        )}
      >
        <ul className="flex min-h-0 flex-col gap-4 overflow-hidden">
          {NAV_LINKS.map((link) => (
            <li key={link.href}>
              <a
                href={link.href}
